...
Improve DOMPurify sanitization throughput by 10%+ (measured in operations per second) without breaking tests or compromising security. Target: baseline ~1800 ops/sec → 1980+ ops/sec.

## What you CAN modify

- `src/*.ts` — TypeScript source files (purify.ts, utils.ts, config.ts, tags.ts, attrs.ts, regexp.ts)

## What you CANNOT modify

- `PROGRAM.md` — research program specification
- `PREPARE.md` — evaluation setup and trust boundary
- `.polyresearch/` — runtime directory (contains benchmark harness)
- `test/` — test suite and fixtures (these define correctness)
